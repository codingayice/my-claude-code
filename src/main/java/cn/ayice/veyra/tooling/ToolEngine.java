...
import cn.ayice.veyra.tooling.permission.PermissionUpdate;
import cn.ayice.veyra.tooling.permission.PermissionUpdateApplier;
import cn.ayice.veyra.tooling.permission.PermissionUpdateSuggestions;
...
public class ToolEngine {
    ...
    public ToolAuthorization authorize(
        ...
        BaseTool tool = dispatcher == null ? null : dispatcher.getTool(request.name());
        ...
        if (choice == ToolExecutionConfirmation.Choice.DENY) {
            return new ToolAuthorization(request, tool, decision, choice, context, "用户拒绝了工具调用");
        }
        ...
        if (choice == ToolExecutionConfirmation.Choice.ALLOW_FOR_SESSION) {
            ...
            nextContext = PermissionUpdateApplier.apply(context, updates);
    ...
    public ToolExecution execute(
        ...
        try {
            ToolResult result = dispatcher.execute(authorization.request(), context);
            ...
            return new ToolExecution(authorization.request(), result, content);
     ...
     * 创建允许执行的工具授权结果。
     */
    private static ToolAuthorization allowed(
            ToolExecutionRequest request,
            BaseTool tool,
        ...
        return new ToolAuthorization(request, tool, decision, choice, context, null);
    }

    /**
     * 创建包含稳定拒绝原因的工具授权结果。
     */
    private static ToolAuthorization rejected(
            ToolExecutionRequest request,
            BaseTool tool,
            PermissionDecision decision,
            PermissionContext context,
        ...
        return new ToolAuthorization(request, tool, decision, null, context, reason);
    }
}
