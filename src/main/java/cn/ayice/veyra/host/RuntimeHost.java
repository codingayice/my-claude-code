...
import cn.ayice.veyra.kernel.RunCommand;
import cn.ayice.veyra.kernel.RunCoordinator;
import cn.ayice.veyra.kernel.RunMode;
...
public class RuntimeHost {
     ...
     */
    public RuntimeHost(SessionRegistry sessions, RunCoordinator runs) {
    ...
    public SessionState createSession() {
        return sessions.createSession().state();
    }
    ...
    public SessionState session(String sessionId) {
        return sessions.getOrCreate(sessionId).state();
    }
    ...
    public SessionState updateSettings(String sessionId, String workingDir, String permissionMode) {
        return sessions.getOrCreate(sessionId).updateSettings(workingDir, permissionMode);
    }
    ...
    public List<SessionSummary> listSessions() {
        return sessions.listSessions().stream()
    ...
    public List<TranscriptItem> transcriptEntries(String sessionId) {
        return sessions.transcriptEntries(sessionId).stream()
    ...
    public RunSubmission submitRun(String sessionId, String input, String mode) {
        SessionRuntime session = sessions.getOrCreate(sessionId);
    ...
    public List<PendingApprovalState> pendingApprovals(String sessionId) {
        return sessions.getOrCreate(sessionId).pendingApprovals();
    }
    ...
    public boolean resolveApproval(String sessionId, String approvalId, String decision) {
        return sessions.getOrCreate(sessionId).resolveApproval(approvalId, decision);
    }
    ...
    public List<CommandOption> commandOptions(String sessionId, String query) {
        return sessions.getOrCreate(sessionId).commandOptions(query).stream()
    ...
    public Optional<CommandResult> findCommand(String sessionId, String command) {
        return sessions.getOrCreate(sessionId).findCommand(command)
    ...
    public SessionEventStream events(String sessionId) {
        return sessions.getOrCreate(sessionId).events();
    }
}
