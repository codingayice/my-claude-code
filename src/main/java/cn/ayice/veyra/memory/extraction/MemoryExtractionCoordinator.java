...
import cn.ayice.veyra.subagent.AgentProfiles;
...
public class MemoryExtractionCoordinator {
    ...
    private MemoryExtractionRequest pending;
    ...
    public void submit(List<ChatMessage> messages, boolean mainAgentWroteMemory) {
        ...
        synchronized (monitor) {
            ...
            pending = new MemoryExtractionRequest(sessionId, cursor, messages);
            if (!running) {
                running = true;
    ...
    public boolean extractNow(List<ChatMessage> messages) {
        ...
        return execute(new MemoryExtractionRequest(sessionId, cursor, messages));
    }
     ...
     */
    private boolean execute(MemoryExtractionRequest request) {
        if (request.messages().size() <= request.fromMessageIndex()) {
            return true;
        }
        try {
            AgentRunResult result = runtime.run(
                    AgentProfiles.memoryExtraction(maxRounds),
                    buildPrompt(request),
                    null,
     ...
     */
    private static String buildPrompt(MemoryExtractionRequest request) {
