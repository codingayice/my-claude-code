...
import cn.ayice.veyra.conversation.context.systemprompt.ActionsSection;
import cn.ayice.veyra.conversation.context.systemprompt.CommunicationStyleSection;
import cn.ayice.veyra.conversation.context.systemprompt.EnvironmentInfoSection;
import cn.ayice.veyra.conversation.context.systemprompt.IntroSection;
import cn.ayice.veyra.conversation.context.systemprompt.MemoryPolicySection;
import cn.ayice.veyra.conversation.context.systemprompt.ProjectInstructionSection;
import cn.ayice.veyra.conversation.context.systemprompt.SelfDescriptionSection;
import cn.ayice.veyra.conversation.context.systemprompt.SubagentSection;
import cn.ayice.veyra.conversation.context.systemprompt.SystemPromptContext;
import cn.ayice.veyra.conversation.context.systemprompt.SystemPromptRegistry;
import cn.ayice.veyra.conversation.context.systemprompt.TodoPlanningSection;
import cn.ayice.veyra.conversation.context.systemprompt.TokenBudgetSection;
import cn.ayice.veyra.conversation.context.systemprompt.ToolsSection;
import cn.ayice.veyra.conversation.memory.MemoryContext;
...
public class ContextBuilder {
    ...
    private MemoryContext buildMemoryContext(List<WorkingMessage> messages, int userIndex) {
        if (memoryContextBuilder == null || userIndex >= messages.size()
                || !(messages.get(userIndex).message() instanceof UserMessage userMessage)) {
            return MemoryContext.empty();
        }
        try {
            return memoryContextBuilder.build(userMessage.singleText());
