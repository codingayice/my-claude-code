...
export const Confirmation = ({ className, approval, state, ...props }: ConfirmationProps) => {
    ...
    return (
        <ConfirmationContext.Provider value={{ approval, state }}>
            <Alert
                className={cn(
                    "flex w-full items-center justify-between gap-3 rounded-none border-0 bg-transparent px-0 py-1 text-sm shadow-none",
                    className,
                )}
                {...props}
            />
...
export const ConfirmationTitle = ({ className, ...props }: ConfirmationTitleProps) => (
    <AlertDescription
        className={cn(
            "min-w-0 flex-1 text-foreground [&_code]:rounded [&_code]:bg-muted [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:font-mono [&_code]:text-sm",
            className,
        )}
        {...props}
    />
...
export const ConfirmationActions = ({ className, ...props }: ConfirmationActionsProps) => {
    ...
    return (
        <div className={cn("flex shrink-0 items-center justify-end gap-2", className)} {...props} />
    )
}
...
export const ConfirmationAction = (props: ConfirmationActionProps) => (
    <Button className="h-8 rounded-lg px-3 text-sm" type="button" {...props} />
)
